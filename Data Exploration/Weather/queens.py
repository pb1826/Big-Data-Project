...
if __name__ == "__main__":
	...
	queens = data.map(lambda x: (x[0],x[1]));

	#count
	count_queens = queens.map(lambda x: (x[0],1)).reduceByKey(lambda x,y:x+y).collect();
	...
	count_queens=count_queens.map(row2csv);
	count_queens.saveAsTextFile("queens-crime-feb.csv");
