...
if __name__ == "__main__":
    ...
    overall = overall.map(lambda x: (x[2],str(x[3]).replace(',',' ')));
	...
	bronx = bronx.map(lambda x: (x[2],str(x[3]).replace(',',' ')));
	#brooklyn
	...
	brooklyn = brooklyn.map(lambda x: (x[2],str(x[3]).replace(',',' ')));
	#manhattan
	...
	manhattan = manhattan.map(lambda x: (x[2],str(x[3]).replace(',',' ')));
	#queens
	...
	queens = queens.map(lambda x: (x[2],str(x[3]).replace(',',' ')));
	#staten island
	...
	staten = staten.map(lambda x: (x[2],str(x[3]).replace(',',' ')));
