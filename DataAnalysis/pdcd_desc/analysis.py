...
if __name__ == "__main__":
	...
	overall = data.map(lambda x: (x[0],x[1],x[6],x[7],x[8],x[9],x[11],x[13],x[14]));
        overall = overall.map(lambda x: (x[4],str(x[5]).replace(',',' ')));
	...
	bronx = bronx.map(lambda x: (x[4],str(x[5]).replace(',',' ')));
	#brooklyn
	...
	brooklyn = brooklyn.map(lambda x: (x[4],str(x[5]).replace(',',' ')));
	#manhattan
	...
	manhattan = manhattan.map(lambda x: (x[4],str(x[5]).replace(',',' ')));
	#queens
	...
	queens = queens.map(lambda x: (x[4],str(x[5]).replace(',',' ')));
	#staten island
	...
	staten = staten.map(lambda x: (x[4],str(x[5]).replace(',',' ')));
