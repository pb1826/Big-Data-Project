...
if __name__ == "__main__":
	...
	overall = data.map(lambda x: (x[0],x[1],x[6],x[10],x[11],x[13],x[14],x[18]));
        overall = overall.map(lambda x: str(x[7]).replace(',', ' '));
	...
	bronx = bronx.map(lambda x: str(x[7]).replace(',', ' '));
	#brooklyn
	...
	brooklyn = brooklyn.map(lambda x: str(x[7]).replace(',', ' '));
	#manhattan
	...
	manhattan = manhattan.map(lambda x: str(x[7]).replace(',', ' '));
	#queens
	...
	queens = queens.map(lambda x: str(x[7]).replace(',', ' '));
	#staten island
	...
	staten = staten.map(lambda x: str(x[7]).replace(',', ' '));
