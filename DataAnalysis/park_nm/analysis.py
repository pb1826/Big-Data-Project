...
if __name__ == "__main__":
	...
	overall = data.map(lambda x: (x[0],x[1],x[6],x[7],x[10],x[11],x[13],x[14],x[17],x[18]));
        overall = overall.map(lambda x: (str(x[3]).replace(',', ' '),x[8]));
	...
	bronx = bronx.map(lambda x: (str(x[3]).replace(',', ' '),x[8]));
	#brooklyn
	...
	brooklyn = brooklyn.map(lambda x: (str(x[3]).replace(',', ' '),x[8]));
	#manhattan
	...
	manhattan = manhattan.map(lambda x: (str(x[3]).replace(',', ' '),x[8]));
	#queens
	...
	queens = queens.map(lambda x: (str(x[3]).replace(',', ' '),x[8]));
	#staten island
	...
	staten = staten.map(lambda x: (str(x[3]).replace(',', ' '),x[8]));
